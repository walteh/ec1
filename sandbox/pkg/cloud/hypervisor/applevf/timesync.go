...
func watchWakeupNotifications(ctx context.Context, vm *vf.VirtualMachine, vsockPort uint32) {
...
func setupGuestTimeSync(ctx context.Context, vm *vf.VirtualMachine, timesync *config.TimeSync) error {
	if timesync == nil {
		return nil
	}
	...
	go watchWakeupNotifications(ctx, vm, timesync.VsockPort)
