...
// func WrapBenchmark(b *testing.B, func(t testing.TB, input)) {
...
func (sb *StreamBenchmark) WithTiming() *StreamBenchmark {
	sb.timing = true
	return sb
}
		...
		for implName, implFunc := range implementations {
			b.Run(fmt.Sprintf("%s/%s/%s", sb.name, implName, sizeName), func(b *testing.B) {
				...
				for i := 0; i < sb.warmup; i++ {
					...
					result, err := implFunc(b.Context(), input, size)
					if err != nil {
				...
				for i := 0; i < b.N; i++ {
					input := dataGenerator(size)
					if sb.timing {
						input = NewTimingReader(b.Context(), input, fmt.Sprintf("%s-input", implName))
					}
					result, err := implFunc(b.Context(), input, size)
					if err != nil {
					...
					if sb.timing {
						result = NewTimingReader(b.Context(), result, fmt.Sprintf("%s-output", implName))
					}
...
func (sb *StreamBenchmark) RunDetailed(b *testing.B, impl BenchmarkFunc, dataGenerator func(size int) io.Reader) []ComparativeResult {
	...
	for _, size := range sb.sizes {
		...
		b.Run(fmt.Sprintf("%s/detailed/%s", sb.name, sizeName), func(b *testing.B) {
			...
			for i := 0; i < sb.warmup; i++ {
				input := dataGenerator(size)
				result, err := impl(b.Context(), input, size)
				if err != nil {
					b.Fatalf("warmup failed: %v", err)
				}
				io.Copy(io.Discard, result)
